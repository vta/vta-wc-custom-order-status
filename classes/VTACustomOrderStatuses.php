...
class VTACustomOrderStatuses {
	...
	/**
	 * Checks if we are in post page for VTA Custom Order Status CPT.
	 * @return bool
	 */
	private function is_post_screen(): bool {

		[ 'path' => $path, 'query_params' => $query_params ] = get_query_params();

		// check if we are on a post page first
		if ( !preg_match('#(post-new\.php)|(post\.php)#', $path) ) {
			return false;
		}

		// check if post type is for VTA Custom Order Status
		$query_params_keys = array_keys($query_params);
		$is_post_type      = false;
		if ( in_array('post', $query_params_keys) ) { // editing posts...
			$post_id      = $query_params['post'];
			$is_post_type = get_post_type($post_id) === $this->post_type;

		} elseif ( in_array('post_type', $query_params_keys) ) { // new posts...
			$post_type    = $query_params['post_type'];
			$is_post_type = $post_type === $this->post_type;
		}
		return $is_post_type;
	}
